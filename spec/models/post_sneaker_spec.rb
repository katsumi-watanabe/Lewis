...
RSpec.describe PostSneaker,  type: :model do
  describe 'モデルのテスト' do
  ...
  context "nameカラム" do
    ...
    it '30文字を越えたらエラーメッセージ' do
      ...
      post_sneaker.sneakers_name = Faker::Lorem.characters(number: 31)
      post_sneaker.valid?
  ...
  context "imageカラム" do
    it "空欄であればエラーメッセージがかえる" do
      post_sneaker = build(:post_sneaker, post_image: nil)
      post_sneaker.valid?
      expect(post_sneaker.errors[:post_image]).to include("を選択してください")
    end
  end
  ...
  context "captionカラム" do
    ...
    it '1文字以上でOK' do
      post_sneaker = create(:post_sneaker)
      post_sneaker.caption = Faker::Lorem.characters(number: 0)
      post_sneaker.valid?
      expect(post_sneaker.errors[:caption]).to include("は1文字以上で入力してください")
    end
    ...
    it '90文字を越えたらエラーメッセージ' do
      post_sneaker = create(:post_sneaker)
      post_sneaker.caption = Faker::Lorem.characters(number: 91)
      post_sneaker.valid?
